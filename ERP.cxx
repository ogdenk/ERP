...
int main(int argc, char * argv[])
{
  ...
  //  1 x 1 x 1 mm.  We will just use the label geometry to do this
  ...
  ResampleFilterType::Pointer labelResampler = ResampleFilterType::New();
  ...
  // loop over the 8 possible configurations of the pipeline to generate the features
  for (k = 0; k < 8; k++)
  {
	  // Use a switch statement to configure the pipeline for the 8 possible cases
	  switch (k)
	  {
	  case 0:
	  ...
	  while (labelNamesIterator != labelNames.end())
	  {
		  ...
		  for (j = 0; j < binaryImageToShapeLabelMapFilter->GetOutput()->GetNumberOfLabelObjects(); j++)
		  {
			  ...
			  fprintf(outputFiles[, outputstring.c_str(), labelObject->GetRoundness());
